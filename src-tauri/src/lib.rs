...
fn initialize_core_logic(app_handle: &AppHandle) {
    ...
    // Warm the selected model at startup so the first dictation after launch
    // doesn't pay the load cost (the hotkey path only starts loading when
    // recording begins, so short first dictations stall on the load).
    // Skipped when the user chose "Immediately" unloading or when the model
    // isn't downloaded yet (e.g. during onboarding). If the user never
    // dictates, the idle watcher unloads it per the configured timeout.
    {
        let settings = settings::get_settings(app_handle);
        let model_ready = !settings.selected_model.is_empty()
            && model_manager
                .get_model_info(&settings.selected_model)
                .is_some_and(|m| m.is_downloaded);
        if model_ready
            && !settings.stt_cloud_enabled
...
pub fn run(cli_args: CliArgs) {
    ...
    let specta_builder = Builder::<tauri::Wry>::new()
        .commands(collect_commands![
            ...
            shortcut::change_recording_start_sound_setting,
            shortcut::change_recording_stop_sound_setting,
            ...
            commands::update::get_local_update_status,
            commands::update::run_local_update,
            commands::initialize_enigo,
            commands::initialize_shortcuts,
            commands::models::get_available_models,
            commands::models::get_model_info,
            commands::stt::get_stt_providers,
            commands::stt::change_stt_cloud_enabled_setting,
            commands::stt::change_stt_provider_setting,
            ...
            commands::audio::play_feedback_preview,
        ...
        .events(collect_events![
            ...
            commands::update::LocalUpdateProgress,
        ]);
    ...
    // Single-instance forwards CLI args to an already-running VoiceApp and exits.
