...
pub fn get_icon_path(theme: AppTheme, state: TrayIconState) -> &'static str {
    match (theme, state) {
        ...
        (AppTheme::Colored, TrayIconState::Idle) => "resources/voiceapp.png",
...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("VoiceApp v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("VoiceApp v{}", env!("CARGO_PKG_VERSION"))
    }
}
...
pub fn update_tray_menu(app: &AppHandle, state: &TrayIconState, locale: Option<&str>) {
    ...
    downloaded.sort_by(|a, b| match (a.is_custom, b.is_custom) {
...
mod tests {
    ...
    fn build_entry(transcription: &str, post_processed: Option<&str>) -> HistoryEntry {
        HistoryEntry {
            id: 1,
            file_name: "voiceapp-1.wav".to_string(),
            timestamp: 0,
            saved: false,
