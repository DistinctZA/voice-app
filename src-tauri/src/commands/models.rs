...
pub async fn has_any_models_available(
    ...
    // Cloud transcription counts as a usable model — don't force cloud-only
    // users back through local model onboarding.
    if get_settings(&app_handle).stt_cloud_enabled {
        return Ok(true);
    }
