...
fn init_gtk_layer_shell(overlay_window: &tauri::webview::WebviewWindow) -> bool {
    if env_flag_enabled("VOICEAPP_NO_GTK_LAYER_SHELL") {
        debug!("Skipping GTK layer shell init (VOICEAPP_NO_GTK_LAYER_SHELL is enabled)");
        return false;
    }
...
// Bumped on every show so a pending delayed hide (see
// hide_recording_overlay) can detect that the overlay was re-shown in the
// meantime and must not hide the window out from under the new recording.
...
/// Shows the overlay in its "opening the microphone" state.
///
/// Opening an input stream can take most of a second on some interfaces, and
/// until it completes nothing the user says is captured. Showing the live
/// recording state during that window tells the user to start talking into a
/// microphone that isn't listening yet, so the wait gets its own state and
/// [`mark_overlay_recording`] promotes it once the stream is actually up.
pub fn show_starting_overlay(app_handle: &AppHandle) {
    show_overlay_state(app_handle, "starting");
}

/// Switches an already-visible overlay to the live recording state without
/// re-showing or repositioning the window.
pub fn mark_overlay_recording(app_handle: &AppHandle) {
    if let Some(overlay_window) = app_handle.get_webview_window("recording_overlay") {
...
pub fn hide_recording_overlay(app_handle: &AppHandle) {
    ...
    if let Some(overlay_window) = app_handle.get_webview_window("recording_overlay") {
        ...
        // If the hotkey fires again during that delay the overlay is re-shown
        // (bumping OVERLAY_EPOCH), and this stale timer must become a no-op —
        // otherwise rapid hotkey presses get their overlay hidden mid-recording.
