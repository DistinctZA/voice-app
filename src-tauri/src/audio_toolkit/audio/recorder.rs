...
impl AudioRecorder {
    ...
    pub fn open(&mut self, device: Option<Device>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let worker = std::thread::spawn(move || {
            ...
            let init_result = (|| -> Result<(cpal::Stream, u32), String> {
                let config = AudioRecorder::resolve_config(&thread_device)
        ...
        match init_rx.recv() {
            ...
            Ok(Err(error_message)) => {
                let _ = worker.join();
                // The device may have come back with a different config (or
                // gone away entirely) — don't keep serving the stale one.
                Self::forget_cached_config(&device);
    ...
    /// Cached per-device stream config. Resolving one costs two CoreAudio
    /// round trips (`default_input_config` + `supported_input_configs`,
    /// ~40ms combined) and the answer is a property of the hardware, so it is
    /// resolved once per device name and reused on subsequent opens. Cleared
    /// when a device fails to open, in case it came back with a new config.
    fn config_cache() -> &'static Mutex<HashMap<String, cpal::SupportedStreamConfig>> {
        static CACHE: OnceLock<Mutex<HashMap<String, cpal::SupportedStreamConfig>>> =
            OnceLock::new();
        CACHE.get_or_init(|| Mutex::new(HashMap::new()))
    }

    fn cached_config(device: &cpal::Device) -> Option<cpal::SupportedStreamConfig> {
        let name = device.name().ok()?;
        Self::config_cache().lock().ok()?.get(&name).cloned()
    }

    fn cache_config(device: &cpal::Device, config: &cpal::SupportedStreamConfig) {
        if let Ok(name) = device.name() {
            if let Ok(mut cache) = Self::config_cache().lock() {
                cache.insert(name, config.clone());
            }
        }
    }

    fn forget_cached_config(device: &cpal::Device) {
        if let Ok(name) = device.name() {
            if let Ok(mut cache) = Self::config_cache().lock() {
                cache.remove(&name);
            }
    ...
    fn resolve_config(
        ...
        if let Some(config) = Self::cached_config(device) {
            return Ok(config);
        }
        let config = Self::get_preferred_config(device)?;
        Self::cache_config(device, &config);
        Ok(config)
    }
...
fn run_consumer(
    ...
    loop {
        ...
        // Apply pending commands around this chunk rather than after it.
        // `Start` has to land first, otherwise the buffer that arrives
        // immediately after the hotkey is processed with `recording == false`
        // and silently dropped — the very beginning of the dictation. `Stop`
        // has to land last, since this chunk was captured before the key was
        // released and belongs in the recording.
        let mut pending_stop: Option<mpsc::Sender<Vec<f32>>> = None;
        ...
        // The deferred stop runs first, then any command that arrived since.
        let mut deferred = pending_stop.take().map(Cmd::Stop);
        loop {
            let cmd = match deferred.take().or_else(|| cmd_rx.try_recv().ok()) {
                Some(cmd) => cmd,
                None => break,
            };
            match cmd {
