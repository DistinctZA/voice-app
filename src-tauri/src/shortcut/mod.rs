...
use crate::settings::{
    ...
    PasteMethod, ShortcutBinding, TypingTool,
};
use crate::tray;
...
pub fn change_binding(
    ...
    if let Err(e) = register_shortcut(&app, updated_binding.clone()) {
        ...
        // Attempt to restore the previous binding so the shortcut keeps working
        if let Err(restore_err) = register_shortcut(&app, previous_binding.clone()) {
            error!(
                "change_binding rollback error for '{}': {}",
                previous_binding.current_binding, restore_err
            );
        }
...
#[tauri::command]
#[specta::specta]
pub fn change_recording_start_sound_setting(
    app: AppHandle,
    sound_id: String,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.recording_start_sound = crate::feedback_sounds::normalize_start_sound(&sound_id);
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_recording_stop_sound_setting(app: AppHandle, sound_id: String) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.recording_stop_sound = crate::feedback_sounds::normalize_stop_sound(&sound_id);
    settings::write_settings(&app, settings);
    Ok(())
}
