...
impl HandyKeysState {
    ...
    pub fn new(app: AppHandle) -> Result<Self, String> {
        ...
        let (startup_tx, startup_rx) = mpsc::channel::<ManagerStartup>();
        ...
        match startup_rx.recv_timeout(std::time::Duration::from_secs(5)) {
            Ok(ManagerStartup::Ready) => Ok(Self {
            ...
            Ok(ManagerStartup::Failed(message)) => Err(message),
            Err(_) => Err(
                "Timed out waiting for handy-keys to start. Check Accessibility permission."
                    .to_string(),
            ),
        }
    }
    ...
    fn manager_thread(
        ...
        app: AppHandle,
        startup_tx: Sender<ManagerStartup>,
    ) {
        ...
        let manager = match HotkeyManager::new_with_blocking() {
            Ok(m) => {
                let _ = startup_tx.send(ManagerStartup::Ready);
                m
            }
            Err(e) => {
                let message = format!(
                    "Accessibility permission not granted. Please enable VoiceApp in System Settings > Privacy & Security > Accessibility: {e}"
                );
                error!("Failed to create HotkeyManager: {}", message);
                let _ = startup_tx.send(ManagerStartup::Failed(message));
                return;
            }
        };
...
pub fn init_shortcuts(app: &AppHandle) -> Result<(), String> {
    ...
    for (id, default_binding) in default_bindings {
        ...
        if let Err(e) = state.register(&binding) {
            ...
            if id == "transcribe" {
                return Err(format!("Failed to register transcribe shortcut: {e}"));
            }
        }
    }
