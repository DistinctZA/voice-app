...
const settingUpdaters: {
  ...
  recording_start_sound: (value) =>
    commands.changeRecordingStartSoundSetting(value as string),
  recording_stop_sound: (value) =>
    commands.changeRecordingStopSoundSetting(value as string),
...
export const useSettingsStore = create<SettingsStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    playFeedbackPreview: async (
      soundType: "start" | "stop",
      soundId: string,
    ) => {
      try {
        await commands.playFeedbackPreview(soundType, soundId);
      } catch (error) {
        console.error(
          `Failed to preview ${soundType} sound (${soundId}):`,
          error,
        );
      }
    },
