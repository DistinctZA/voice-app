...
const ModelSelector: React.FC<ModelSelectorProps> = ({ onError }) => {
  ...
  const cloudModel = cloudEnabled
    ? (settings?.stt_models?.[settings?.stt_provider_id ?? ""] ?? "")
    : "";
  ...
  return (
    <>
      ...
      <div className="relative" ref={dropdownRef}>
        <ModelStatusButton
          status={cloudEnabled ? "ready" : getDisplayStatus()}
          displayText={
            cloudEnabled
              ? `${t("settings.models.cloud.activeBadge")} · ${cloudModel}`
              : getModelDisplayText()
          }
