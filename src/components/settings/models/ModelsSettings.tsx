...
import { CloudTranscription } from "@/components/settings/CloudTranscription";
import { CloudIcon } from "@/components/icons";
import { useSettings } from "@/hooks/useSettings";
import { useModelStore } from "@/stores/modelStore";
import { LANGUAGES } from "@/lib/constants/languages.ts";
import { compareModelsByQuality, isCatalogModel } from "@/lib/constants/models";
...
export const ModelsSettings: React.FC = () => {
  ...
  const filteredModels = useMemo(() => {
    return models.filter((model: ModelInfo) => {
      if (!isCatalogModel(model)) return false;
      if (languageFilter !== "all") {
  ...
  const { downloadedModels, availableModels } = useMemo(() => {
    ...
    // Sort: active model first, then by quality, custom models last
    downloaded.sort((a, b) => {
      if (a.id === currentModel) return -1;
      if (b.id === currentModel) return 1;
      return compareModelsByQuality(a, b);
    });

    available.sort(compareModelsByQuality);
  ...
  return (
    <div className="max-w-3xl w-full mx-auto space-y-4">
      ...
      <CloudTranscription />
      {cloudEnabled && (
        <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl border border-logo-primary/30 bg-logo-primary/10 text-sm">
          <CloudIcon width={18} height={18} className="shrink-0" />
          <span>{t("settings.models.cloud.localDisabled")}</span>
        </div>
      )}
      <div
        className={
          cloudEnabled ? "opacity-40 transition-opacity" : "transition-opacity"
        }
      >
