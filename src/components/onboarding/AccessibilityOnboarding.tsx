...
const AccessibilityOnboarding: React.FC<AccessibilityOnboardingProps> = ({
  ...
  const recheckPermissions = useCallback(async (): Promise<boolean> => {
    ...
    // macOS can lag updating TCC; verify by initializing input if needed.
    if (!accessibilityGranted) {
      try {
  ...
  }, [hasWindowsMicrophoneAccess, permissionPlatform]);
    ...
    pollingRef.current = setInterval(async () => {
      try {
        ...
        setPermissions((prev) => {
          ...
          if (accessibilityOk && prev.accessibility !== "granted") {
        ...
        if (accessibilityOk && microphoneGranted) {
          if (pollingRef.current) {
            clearInterval(pollingRef.current);
            pollingRef.current = null;
          }
  ...
  // Keep checking while permissions are still pending.
  useEffect(() => {
    if (permissionPlatform !== "macos" && permissionPlatform !== "windows") {
      return;
    }
    if (
                ...
                {permissions.accessibility === "granted" ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <p className="text-sm text-text/60">
                      {t("onboarding.permissions.accessibility.manualHint")}
                    </p>
                    <button
                      type="button"
                      onClick={() =>
                        commands.revealExecutableInFinder().catch(console.error)
                      }
                      className="px-4 py-2 rounded-lg border border-mid-gray/30 hover:bg-mid-gray/10 text-sm font-medium transition-colors"
                    >
                      {t(
                        "onboarding.permissions.accessibility.revealExecutable",
                      )}
                    </button>
                    <button
                      type="button"
